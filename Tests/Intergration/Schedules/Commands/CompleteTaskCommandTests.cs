...
public class CompleteTaskCommandTests
{
    ...
    [Test]
    public async Task CompleteTaskCommandHandler_Success()
    {
        var user = new User();
        ...
        var sut = new CompleteTaskCommandHandler(
                _identityMock,
                _checklistRepo,
                _userRepo,
                _unitOfWork);
    ...
    [Test]
    public async Task CompleteTaskCommandHandler_Fail_WhenUserDoesNotOwnChecklist()
    {
        var user = new User();
        user.Create(CurrentUserId, new(CurrentUserName), _fix.Create<PasswordHash>());
        await _userRepo.Save(user, default);
    ...
    [Test]
    public async Task CompleteTaskCommandHandler_Fail_WhenChecklistDoesNotExist()
    {
        var user = new User();
        user.Create(CurrentUserId, new(CurrentUserName), _fix.Create<PasswordHash>());
        await _userRepo.Save(user, default);
