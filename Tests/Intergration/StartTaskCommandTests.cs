...
[TestFixture]
public class StartTaskCommandTests
{
    ...
    [Test]
    public async Task StartTaskCommandHandler_Success()
    {
        var user = new User();
        ...
        var sut = new StartTaskCommandHandler(
                _identityMock,
                _checklistRepo,
                _userRepo,
                _unitOfWork);
        ...
        var task = storedChecklist.Tasks.First(t => t.Id == taskId);
        Assert.That(task, Is.Not.Null);
        Assert.That(task.IsStarted(), Is.True);
    }

    [Test]
    public async Task StartTaskCommandHandler_FailWhenUserDoesNotExist()
    {
        var checklist = new Checklist();
        var checklistId = new ChecklistId(Guid.NewGuid());
        checklist.Create(checklistId, CurrentUserId);
    ...
    [Test]
    public async Task StartTaskCommandHandler_Fail_WhenUserDoesNotOwnChecklist()
    {
        var user = new User();
        user.Create(CurrentUserId, new(CurrentUserName), _fix.Create<PasswordHash>());
        await _userRepo.Save(user, default);
    ...
    [Test]
    public async Task StartTaskCommandHandler_Fail_WhenChecklistDoesNotExist()
    {
        var user = new User();
        user.Create(CurrentUserId, new(CurrentUserName), _fix.Create<PasswordHash>());
        await _userRepo.Save(user, default);
