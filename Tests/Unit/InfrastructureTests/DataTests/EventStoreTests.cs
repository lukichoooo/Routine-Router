...
public class EventStoreTests
{
    ...
    {
        // Arrange
        var context = await TestFactory.GetEventContextAsync();
        ...
        var onDbEvent = Event.From(
                responseEvent,
                EventMapper.ToPayload(responseEvent));

        await context.AddAsync(onDbEvent);
        await context.SaveChangesAsync();

        IEventStore sut = new SQLiteEventStore(context);

        // Act
        var res = await sut.Load(aggregateId: aggregateId, ct: default);
    ...
    public async Task LoadAsyncTest_FromVersion(int fromVersion, int? toVersion)
    {
        ...
        var onDbEvents = responseEvents.Select(e =>
                Event.From(e, EventMapper.ToPayload(e)));
